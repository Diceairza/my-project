...
import { FileTextIcon } from '../../components/icons/LucideIcons';
...
const AgedReceivablesReport: React.FC<AgedReceivablesReportProps> = ({ data, asOfDate }) => {
  const currency = data.length > 0 ? data[0].currency : '';

  const columns = [
    { Header: 'Customer', accessor: 'customerOrSupplierName' as keyof AgedDocument, className: 'font-medium' },
    { Header: 'Invoice #', accessor: 'documentNumber' as keyof AgedDocument },
    { Header: 'Issue Date', accessor: 'issueDate' as keyof AgedDocument, Cell: (date:string) => new Date(date).toLocaleDateString() },
  ...
  return (
    <div className="space-y-4 p-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 mb-6 text-center">
       ...
       <div className="p-3 bg-blue-50 rounded-lg shadow-sm text-center mb-6">
      ...
      {data.length === 0 ? (
        <div className="text-center py-10 bg-white rounded-lg shadow">
            ...
            <p className="text-gray-500">No outstanding receivables found for the selected period.</p>
        </div>
      ) : (
